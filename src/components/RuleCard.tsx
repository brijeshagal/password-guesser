...
interface RuleCardProps {
  rule: Rule & { isPassed: boolean };
  theme: Theme;
}

export default function RuleCard({ rule, theme }: RuleCardProps) {
  return (
    <div
      className="flex flex-col p-3 sm:p-4 rounded-lg border-2 transition-all duration-300 shadow-sm"
      style={{
        borderColor: rule.isPassed ? theme.colors.success : theme.colors.border,
        backgroundColor: rule.isPassed
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div className="flex items-start space-x-2 sm:space-x-3 flex-1 min-w-0">
          <div
            ...
            style={{
              ...
              transform: rule.isPassed ? "scale(110%)" : "scale(100%)",
            }}
          >
            {rule.id}
          </div>
        ...
        <div
          ...
          style={{
            color: rule.isPassed ? theme.colors.success : theme.colors.error,
            transform: rule.isPassed ? "scale(110%)" : "scale(100%)",
          }}
        >
          {rule.isPassed ? "✅" : "❌"}
      ...
      {!rule.isPassed && rule.metadata?.sponsorLogos && (
        <div
        ...
        >
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3">
            {rule.metadata.sponsorLogos.map((sponsor: { name: string; logo: string }, index: number) => (
          ...
          <p
            ...
            style={{ color: theme.colors.textSecondary }}
          >
            💡 Look at the logos above and include one of the sponsor names in
            your password
          </p>
        </div>
      )}
